...
- Added explicit Single and Global automatic-volume bindings. Single follows the actual APO playback endpoint; Global makes all instances follow the current Windows default Multimedia playback volume for routing systems such as VB-Audio Matrix. A missing endpoint, a failed rebind, a capture or unknown flow, or an unreadable volume source fails closed: correction fades to the uncorrected `A = L + H` path over 10 ms, then recovery silently warms for 250 ms and fades correction back in over 100 ms.
