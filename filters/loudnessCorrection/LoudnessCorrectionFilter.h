...
public:
	...
	struct FilterParameters
	{
		...
		template<typename T> bool deSerialize(const T& parameters)
		{
			...
			int error = 0;
			error += archive.get(state, std::wregex(L"\\s*State\\s+(0|1)", std::regex_constants::icase));
			error += archive.get(referenceLevel,
				std::wregex(L"\\s*ReferenceLevel\\s+([-+]?(?:[0-9]+(?:[\\.,][0-9]*)?|[\\.,][0-9]+))",
					std::regex_constants::icase));
			error += archive.get(referenceOffset,
				std::wregex(L"\\s*ReferenceOffset\\s+([-+]?(?:[0-9]+(?:[\\.,][0-9]*)?|[\\.,][0-9]+))",
					std::regex_constants::icase));

			// Attenuation and Volume are optional for backward compatibility.
			int attenuationError = archive.get(attenuation,
				std::wregex(L"\\s*Attenuation\\s+([-+]?(?:[0-9]+(?:[\\.,][0-9]*)?|[\\.,][0-9]+))",
					std::regex_constants::icase));
			if (attenuationError != 0)
				attenuation = 1.0f;

			int volumeError = archive.get(manualVolume,
				std::wregex(L"\\s*Volume\\s+([-+]?(?:[0-9]+(?:[\\.,][0-9]*)?|[\\.,][0-9]+))",
					std::regex_constants::icase));
			useManualVolume = volumeError == 0;
			if (!useManualVolume)
				manualVolume = 0.0f;

			// The immediately preceding release used a different, versioned
			// loudness model. Make its entries editable after an upgrade without
			// reusing their response: show a safe, disabled old-format draft.
			bool isPreviousModel = archive.find(std::wregex(
				L"\\s*Version\\s+3(?:\\s|$)", std::regex_constants::icase)) &&
				archive.find(std::wregex(
					L"\\s*NeutralVolumeDb\\s+", std::regex_constants::icase));
			if (error != 0 && isPreviousModel)
			{
				state = false;
				referenceLevel = 80.0f;
				referenceOffset = 0.0f;
				attenuation = 1.0f;
				int manualVolumeError = archive.get(manualVolume,
					std::wregex(L"\\s*ManualVolumeDb\\s+([-+]?(?:[0-9]+(?:[\\.,][0-9]*)?|[\\.,][0-9]+))",
						std::regex_constants::icase));
				useManualVolume = manualVolumeError == 0;
				if (!useManualVolume)
					manualVolume = 0.0f;
				normalize();
				return false;
...
private:
	...
	static constexpr double HEADROOM_MARGIN_DB = 0.1;
	static const unsigned FIT_ITERATIONS = 4;
	static const unsigned RESPONSE_SCAN_POINTS = 256;
	...
	std::vector<std::vector<BiQuad>> _biquadBanks; // [channel][band]
